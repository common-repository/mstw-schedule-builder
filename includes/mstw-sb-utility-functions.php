 ...
 *
 * 3. mstw_sb_admin_notice - Displays admin notices
 *
 * 4. mstw_sb_add_admin_notice - Adds admin notices to transient for display
 *
 * 5. mstw_sb_count_league_teams - returns the number of teams in a league
 *
 * 6. mstw_sb_get_league_iist - Returns array of leagues of appropriate size
 * 
...
//----------------------------------------------------------------
// 3. mstw_sb_admin_notice - Displays all admin notices; 
...
//					(then deletes it)
//
if ( !function_exists ( 'mstw_sb_admin_notice' ) ) {
	function mstw_sb_admin_notice( ) {
		
		$transient = 'mstw_sb_admin_notices';
	...
	} //End: function mstw_sb_admin_notice( )
}

//----------------------------------------------------------------
// 4. mstw_sb_add_admin_notice - Adds admin notices to transient 
...
//
if ( !function_exists ( 'mstw_sb_add_admin_notice' ) ) {
	function mstw_sb_add_admin_notice( $type = 'updated', $notice ) {
	...
	} //End: function mstw_sb_add_admin_notice( )
}
...
//
if( !function_exists( 'mstw_sb_count_league_teams' ) ) {
	function mstw_sb_count_league_teams( $league_slug = null ) {
...
} //End: mstw_sb_count_league_teams( )

// ------------------------------------------------------------------------------
// 6. mstw_sb_get_league_iist - Returns array of leagues of appropriate size

// CAN USE LEAGUES ARE IN EXISTING FUNCTION. CHANGE THIS TO GET THE LEAGUES ( $size_limit )


//
//	ARGUMENTS: 
//		$maxSize: maximum size of league (0 for no size limit)
//	
//	RETURNS:
//		Returns an array of league slugs, or -1 if no leagues are found
//		
if ( !function_exists( 'mstw_sb_get_league_list' ) ) {
	function mstw_sb_get_league_list( $maxSize ) {
		...
		//
		// $term->count does not match mstw_sb_count_league_teams() above ODD
		//
		foreach( $terms as $term ) {
			$leagueSlug = $term -> slug;
			$leagueSize = mstw_sb_count_league_teams( $leagueSlug );
			
			if ( 2 < $leagueSize && $leagueSize <= $maxSize ) {
				$leagues[] = $leagueSlug;
			} 
		}
	...
	} //End: mstw_sb_get_league_list()
}


// ----------------------------------------------------------------
// 5. mstw_sb_modDivide - find a/b modulo m
//		Arguments:
//			$a - numerator
//			$b - demominator
//      $m - modulus
//		Returns:
//			a/b modulo m if it exists, -1 otherwise
//
if( !function_exists( 'mstw_sb_modDivide' ) ) {
	function mstw_sb_modDivide($a, $b, $m) { 
	
		$a = $a % $m; 
		
		$inv = mstw_sb_modInverse($b, $m);
		
		if ( -1 == $inv ) { 
			$ret_val = -1; //Division not defined";
	...
	} //End: mstw_sb_modDivide( )
}


// ----------------------------------------------------------------
// 5. mstw_sb_modInverse - find the modulo inverse of b 
//		Arguments:
//			$b - number to invert
//      $m - modulus
//		Returns:
//			modulo inverse of b if it exists, -1 otherwise
//
if( !function_exists( 'mstw_sb_modInverse' ) ) {
	function mstw_sb_modInverse( $b, $m ) {
		
		$x = 0;
		$y = 0;  // used in extended GCD algorithm
		$g = mstw_sb_gcdExtended( $b, $m, $x, $y );
	...
	} //End: mstw_sb_modInverse( )
} 

// ----------------------------------------------------------------
// 5. mstw_sb_gcdExtended - for extended Euclidean Algorithm 
//		Arguments:
//			$b - number to invert
//      $m - modulus
//		Returns:
//			modulo inverse of b if it exists, -1 otherwise
//
if( !function_exists( 'mstw_sb_gcdExtended' ) ) {
	function mstw_sb_gcdExtended( $a, $b, &$x, &$y ) {
    ...
    $gcd = mstw_sb_gcdExtended($b % $a, $a, $x1, $y1); 
...
if( !function_exists( 'mstw_validate_url' ) ) {
	function mstw_validate_url( $data_array, $key, $post_id, $notice_type = 'error', $notice = 'Invalid URL:' ) {
		...
		if( empty( $url ) or mstw_is_valid_url( $url ) ) {
			...
			$notice .= ' ' . $url;			
			if ( function_exists( 'mstw_sb_add_admin_notice' ) ) {
				mstw_sb_add_admin_notice( 'mstw_admin_messages', $notice_type, $notice );
			}
		}
